...
public class MyTasksView {
    ...
    public MyTasksView(User user, MainApp mainApp) {
    ...
    private VBox createHeaderSection() {
        ...
        Text subtitleText = new Text("Hello " + currentUser.getDisplayName() + // Menggunakan getDisplayName
    ...
    private VBox createTasksSection() {
        ...
        Button refreshBtn = createStyledButton("🔄 Refresh", "#3498db", 100, e -> refreshTasks()); // Menggunakan handler
    ...
    private HBox createActionButtons() {
        ...
        Button updateProgressBtn = createStyledButton("📝 Update Progress", "#f39c12", 140, e -> updateProgress()); // Menggunakan handler
        
        Button reportIssueBtn = createStyledButton("⚠️ Report Issue", "#e74c3c", 120, e -> reportIssue()); // Menggunakan handler
        
        Button viewDetailsBtn = createStyledButton("👁️ View Details", "#3498db", 120, e -> viewTaskDetails()); // Menggunakan handler
        ...
        dialog.showAndWait().ifPresent(progress -> {
            ...
            try {
                if (manufacturingOps.updateProductionProgress(task.getOrderId(), progress, notes)) {
                    ...
                    showError("Error", "Failed to update progress (Operation failed).");
                }
            } catch (SQLException e) {
                showError("Database Error", "Failed to update progress: " + e.getMessage());
            }
        });
    }
    ...
    private void refreshTasks() {
        try {
            // Get all orders and filter for tasks assigned to current user
            List<ProductionOrder> allOrders = manufacturingOps.getAllProductionOrders();
            List<ProductionOrder> myTasks = allOrders.stream()
                .filter(order -> {
                    // Filter based on assigned operator or customer name (if applicable)
                    // Assuming assignedOperator is the primary filter for "My Tasks"
                    return currentUser.getUsername().equals(order.getAssignedOperator());
                })
    ...
    private void updateStatistics(List<ProductionOrder> tasks) {
        ...
        int completed = (int) tasks.stream().filter(ProductionOrder::isCompleted).count(); // Menggunakan isCompleted()
        int overdue = (int) tasks.stream().filter(ProductionOrder::isOverdue).count(); // Menggunakan isOverdue()
